...
tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["👷 Mano de Obra", "🚜 Equipos", "📈 Eficiencia", "🧹 Calidad de datos", "🔎 Códigos por Persona"])
...
with tab1:
    ...
    semanal_mo = (
        ...
        .groupby(["semana_lunes", "semana_etiqueta", "frente", "codfrente"], as_index=False)
    ...
    if semanal_mo.empty:
        ...
        matriz_mo = semanal_mo.pivot_table(
            index="semana_etiqueta", columns="codfrente", values="costo_semana",
        ...
        matriz_horas = semanal_mo.pivot_table(
            index="semana_etiqueta", columns="codfrente", values="horas_semana",
            aggfunc="sum", fill_value=0,
        ).reindex(orden_semanas)
...
with tab5:
    ...
    if not nombres_disp:
        ...
        c2.metric("Frentes distintos", datos_persona["codfrente"].nunique())
        c3.metric("Actividades distintas", datos_persona["codact"].nunique())

        resumen_persona = (
            datos_persona
            .groupby(["codfrente", "frente", "codact", "actDesc"], as_index=False)
            .agg(horas=("horas", "sum"), registros=("id", "count"))
            .sort_values(["codfrente", "codact"])
            .rename(columns={
                "codfrente": "Cód. Frente", "frente": "Frente", "codact": "Cód. Actividad",
